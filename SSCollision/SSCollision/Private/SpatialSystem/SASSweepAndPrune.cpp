...
{
	if (CollTarget->GetIncludedSAS() == this)
	{
		int32 ProxyIdx = CollTarget->GetSASProxyIdx();
		CIProxy CIProxy = _CIProxies[ProxyIdx];
		if (CIProxy.Coll != CollTarget)
		{
			SS_INTERRUPT();
			return;
		}

		const int32 StartIdx = CIProxy.SAPListStartIdx + 1;
		const int32 EndIdx = CIProxy.SAPListEndIdx;
		for (int32 i=StartIdx;i<EndIdx;i++)
		{
			const int32 CollidableProxyIdx = _SAPList[i].ProxyIdx;
			ICollInstanceBase* CollidableProxy = _CIProxies[CollidableProxyIdx].Coll;
			ListPushBackUnique(OutList, CollidableProxy);
		}
	}
	else
	{
